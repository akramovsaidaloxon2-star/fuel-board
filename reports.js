(function () {
  ...
  let fuelAgg = null;   // { unit: {qty, amt} }
  let perfAgg = null;   // { vehicle: {miles, idle} }
  let period = { start: null, end: null };
  let computed = null;  // { rows, unmatched, totals }
  let wired = false;
  ...
  function generate() {
    ...
    computed = { rows, unmatched, totals: { qty: +tQty.toFixed(1), amt: +tAmt.toFixed(2), idle: +tIdle.toFixed(1) } };
    render();
  }
  ...
  function render() {
    ...
    tbody.innerHTML = rows.map((r) => `
      <tr>
        ...
        <td class="${r.mpg != null && r.mpg < 6 ? "mpg bad" : "mpg"}">${r.mpg ?? "—"}</td>
        <td>${r.cpm ?? "—"}</td>
        <td class="${r.idleGal >= 30 ? "idle high" : "idle"}">${r.idleGal ?? "—"}</td>
      </tr>`).join("");
  ...
  async function save() {
    ...
    const body = {
      type: $("#rep-type").value, periodStart: period.start, periodEnd: period.end,
      rows: computed.rows, unmatched: computed.unmatched, totals: computed.totals,
    };
  ...
  async function openSaved(id) {
    ...
    if (!r || !r.rows) return;
    computed = { rows: r.rows, unmatched: r.unmatched || [], totals: r.totals || { qty: 0, amt: 0, idle: 0 } };
    period = { start: r.periodStart, end: r.periodEnd };
    if (r.type) $("#rep-type").value = r.type;
    updatePeriodLabel();
    render();
    $("#view-reports").scrollIntoView({ behavior: "smooth" });
  }

  // ---- Export ----
  function exportXlsx() {
    if (!computed) return;
    const data = computed.rows.map((r) => ({
      Unit: r.unit, "Qty (gal)": r.qty, "Amt ($)": r.amt, PPG: r.ppg,
      "Miles": r.miles, MPG: r.mpg, CPM: r.cpm, "Idle gal": r.idleGal,
    }));
